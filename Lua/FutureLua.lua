...
-- UU1: KANOA
--==========================================================================================================================
function FutureEraClearsBarbEncampment(iAttackingPlayer, iAttackingUnit, iAttackerDamage, iAttackerFinalDamage, iAttackerMaxHP, iDefendingPlayer, iDefendingUnit, iDefenderDamage, iDefenderFinalDamage, iDefenderMaxHP, iInterceptingPlayer, iInterceptingUnit, iInterceptorDamage, iPlotX, iPlotY)
	...
	if pAttUnit:IsHasPromotion(GameInfoTypes.PROMOTION_FW_DESTROY_BARBARIAN) and pDefUnit:IsBarbarian() and pPlot:GetImprovementType() == GameInfoTypes.IMPROVEMENT_BARBARIAN_CAMP then
		local KanoaX, KanoaY = pAttUnit:GetX(), pAttUnit:GetY() -- get Kanoa position
		pAttUnit:SetXY(iPlotX, iPlotY)	-- move to encampment plot to clear it
...
function AddPromotionBuildingsFW(iPlayer, iCity, iUnit, bGold, bFaith)
	...
	local tEligibleUnits = {
		GameInfoTypes.UNIT_FW_MEGAHIVE,
		...
		GameInfoTypes.UNIT_FW_HYBRID_DRONE,
		GameInfoTypes.UNIT_FW_BIODRONE
	}

	local tEligibleUraniumUnits = {
		GameInfoTypes.UNIT_FW_MEGAHIVE,
		GameInfoTypes.UNIT_FW_NEXUS,
		GameInfoTypes.UNIT_FW_HYDRA,
		GameInfoTypes.UNIT_FW_SPACE_FIGHTER,
		GameInfoTypes.UNIT_FW_SPACE_BOMBER,
		GameInfoTypes.UNIT_FW_ANGEL,
		GameInfoTypes.UNIT_FW_COLOSSUS,
		GameInfoTypes.UNIT_FW_CRAWLER,
		GameInfoTypes.UNIT_MECH,
		GameInfoTypes.UNIT_FW_HELICARRIER,
		GameInfoTypes.UNIT_FW_SUPERCARRIER,
	}
	...
	local iPromotion4 = GameInfoTypes.PROMOTION_FW_IMPLANTS_IV
	local iImplantCyberClinic = GameInfoTypes.BUILDING_FW_CYBERCLINIC
	...
	local iFusionReactor = GameInfoTypes.BUILDING_FW_FUSION_PLANT
	local iUraniumPromotion = GameInfoTypes.PROMOTION_FW_URANIUM
	...
	if pCity:IsHasBuilding(iImplantCyberClinic) then
		if JFD_GetRandomFW(1, 5) >= 2 then
			local pUnit = pPlayer:GetUnitByID(iUnit)
			for i, iUnitCombatType in pairs(tEligibleCombats) do
				if pUnit and pUnit:GetUnitCombatType() == iUnitCombatType then
					local promotionRoll = JFD_GetRandomFW(1, 4)
					
					if promotionRoll == 1 then
						pUnit:SetHasPromotion(iPromotion4, true)
					elseif promotionRoll == 2 then
						pUnit:SetHasPromotion(iPromotion3, true)
					elseif promotionRoll == 3 then
						pUnit:SetHasPromotion(iPromotion2, true)
					else -- promotionRoll == 4
						pUnit:SetHasPromotion(iPromotion1, true)
					end
				end
			end
		end
	elseif pCity:IsHasBuilding(iImplantClinic) then
        if JFD_GetRandomFW(1, 5) >= 4 then
            local pUnit = pPlayer:GetUnitByID(iUnit)
            for i, iUnitCombatType in pairs(tEligibleCombats) do
                if pUnit and pUnit:GetUnitCombatType() == iUnitCombatType then
					local promotionRoll = JFD_GetRandomFW(1, 3)
	...
	if pCity:IsHasBuilding(iSpaceshipFactory) then
		local pUnit = pPlayer:GetUnitByID(iUnit)
	...
	if pCity:IsHasBuilding(iFusionReactor) then
		local pUnit = pPlayer:GetUnitByID(iUnit)
		local iUnitType = pUnit:GetUnitType()
		for i, iUnitTypeCur in pairs(tEligibleUraniumUnits) do
			if pUnit and iUnitType == iUnitTypeCur then
				pUnit:SetHasPromotion(iUraniumPromotion,true)
				break
			end
		end
	end
...
-- 	[GameInfo.Buildings.BUILDING_FW_APEX_CENTRE.ID] = 3
-- }
