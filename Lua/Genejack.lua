...
local eBuilding = GameInfoTypes.BUILDING_FW_GENEJACK_FACILITY
...
-- aquires new tiles on construction of Genejack Facility, 3 range of city
function OnConstructionClaimResources(iPlayer, iCity, iBuilding)
	local pPlayer = Players[iPlayer]

	if not (pPlayer) then return end
	if iBuilding ~= eBuilding then return end
	...
	for iCityPlot = 1, pCity:GetNumCityPlots() - 1, 1 do
		...
		if iResourceTypeOnTile ~= -1 then
			if iPlotOwner == -1 then
				pSpecificPlot:SetOwner(iPlayer, iCity, 1, 1)
				pPlotClaimer = pPlotClaimer + 1
			elseif pPlayer:GetCivilizationType() == eCivilizationAmerica then
				pSpecificPlot:SetOwner(iPlayer, iCity, 1, 1)
				pPlotClaimer = pPlotClaimer + 1
			end
		end
	...
	-- local iYield = (25 * pPlotClaimer * iEraModifier)
	local iYield = (100 * pPlotClaimer)
