UPDATE Technologies SET Era = 'ERA_FW_FUTURE', GridY = 6, GridX = 18, Cost = 52500 WHERE Type='TECH_NANOTECHNOLOGY';

UPDATE Technologies SET Era = 'ERA_FW_FUTURE', GridY = 7, GridX = 18, Cost = 52500 WHERE Type='TECH_PARTICLE_PHYSICS';

UPDATE Technologies SET GridX = 17, GridY = 5, ExtraVotesPerDiplomat = 0, InternationalTradeRoutesChange = 0, Cost = 39000 WHERE Type='TECH_ROBOTICS';

UPDATE Technologies SET GridX = 16, GridY = 2, ExtraVotesPerDiplomat = 0, Cost = 23000, InternationalTradeRoutesChange = 1 WHERE Type= 'TECH_GLOBALIZATION';

UPDATE Technologies SET Era = 'ERA_FW_FUTURE', GridY = 6, GridX = 22, Cost = 114000 WHERE Type='TECH_NUCLEAR_FUSION';

UPDATE Technologies SET Era = 'ERA_FW_FUTURE', Cost = 162500, GridX = 25 WHERE Type='TECH_FUTURE_TECH';

DELETE FROM Technology_FreePromotions WHERE TechType='TECH_NANOTECHNOLOGY';

-- RESOLUTION_WORLD_IDEOLOGY RESOLUTION_INTERNATIONAL_SPACE_STATION DIPLOMACY VICTORY
UPDATE Resolutions SET TechPrereqAnyMember = 'TECH_COMPUTRONIUM' WHERE Type = 'RESOLUTION_DIPLOMATIC_VICTORY';
UPDATE Resolutions SET TechPrereqAnyMember = 'TECH_DIGITAL_SOCIETY' WHERE Type = 'RESOLUTION_UNITED_NATIONS';
-- UPDATE LeagueSpecialSessions SET EraTrigger = 'ERA_FW_FUTURE' WHERE Type = 'LEAGUE_SPECIAL_SESSION_WORLD_ASSEMBLY';
UPDATE Projects SET Description = "PROMETHEUS PROGRAM", Cost = 12000, TechPrereq = "TECH_PROMETHEUS_PROGRAM" WHERE Type = 'PROJECT_APOLLO_PROGRAM'; 
  
 
------------
-- SPECIAL LIST PROMOTION (TECH) -- CAYM Edited
------------------------------
DELETE FROM Tech_SpecialistYieldChanges
WHERE
    TechType = 'TECH_NUCLEAR_FUSION'
    AND SpecialistType = 'SPECIALIST_SCIENTIST'
    AND YieldType = 'YIELD_SCIENCE'
    AND Yield = 3;

DELETE FROM Tech_SpecialistYieldChanges
WHERE
    TechType = 'TECH_INTERNET'
    AND SpecialistType = 'SPECIALIST_MERCHANT'
    AND YieldType = 'YIELD_GOLD'
    AND Yield = 3;

DELETE FROM Tech_SpecialistYieldChanges
WHERE
    TechType = 'TECH_TELECOM'
    AND SpecialistType = 'SPECIALIST_MUSICIAN'
    AND YieldType = 'YIELD_CULTURE'
    AND Yield = 2;

DELETE FROM Tech_SpecialistYieldChanges
WHERE
    TechType = 'TECH_COMPUTERS'
    AND SpecialistType = 'SPECIALIST_ARTIST'
    AND YieldType = 'YIELD_CULTURE'
    AND Yield = 1;

-- If playing with Enlightenment Era, uncomment this line
-- UPDATE Technologies SET GridX = GridX + 2 WHERE GridX > 10;

INSERT INTO Tech_SpecialistYieldChanges
	(TechType, SpecialistType, YieldType, Yield)
VALUES
    ('TECH_GLOBALIZATION',      'SPECIALIST_MUSICIAN',  'YIELD_CULTURE',    2),
	('TECH_GLOBALIZATION',      'SPECIALIST_MERCHANT',  'YIELD_GOLD',       2),
	('TECH_TELECOM',            'SPECIALIST_ARTIST',    'YIELD_CULTURE',    1),
	('TECH_GRID_COMPUTING',     'SPECIALIST_MERCHANT',  'YIELD_GOLD',       1),
	('TECH_MEMETICS',           'SPECIALIST_ARTIST',    'YIELD_CULTURE',    1),
	-- ('TECH_NANOTECHNOLOGY',     'SPECIALIST_CIVIL_SERVANT',    'YIELD_FAITH',    1),
	('TECH_FW_ECOTHEORY',       'SPECIALIST_WRITER',    'YIELD_CULTURE',    1),
    ('TECH_PARTICLE_PHYSICS',   'SPECIALIST_SCIENTIST', 'YIELD_SCIENCE',    1),
	('TECH_COGNITIVE_TECH',     'SPECIALIST_MUSICIAN',  'YIELD_CULTURE',    1),
	('TECH_PROMETHEUS_PROGRAM', 'SPECIALIST_CITIZEN',   'YIELD_SCIENCE',    1),
    ('TECH_CYBERNETICS',        'SPECIALIST_CITIZEN',   'YIELD_PRODUCTION', 1),
    ('TECH_BIOMODIFICATION',    'SPECIALIST_CITIZEN',   'YIELD_CULTURE',    1),
    ('TECH_NUCLEAR_FUSION',     'SPECIALIST_CITIZEN',   'YIELD_GOLD',       1),
    ('TECH_HOMO_SUPERIOR',      'SPECIALIST_CITIZEN',   'YIELD_TOURISM',    1),
    ('TECH_HOMO_SUPERIOR',      'SPECIALIST_CITIZEN',   'YIELD_GOLDEN_AGE_POINTS', 1),
    ('TECH_COMPUTRONIUM',       'SPECIALIST_CITIZEN',   'YIELD_GREAT_GENERAL_POINTS', 2),
    ('TECH_COMPUTRONIUM',       'SPECIALIST_CITIZEN',   'YIELD_GREAT_ADMIRAL_POINTS', 2),
    ('TECH_SPACE_COLONIZATION', 'SPECIALIST_CITIZEN',   'YIELD_SCIENCE',    2);
	-- ('TECH_CYBERNETICS', 'SPECIALIST_MERCHANT', 'YIELD_GOLD', 3),
	-- ('TECH_PROMETHEUS_PROGRAM', 'SPECIALIST_SCIENTIST', 'YIELD_SCIENCE', 3),
	-- ('TECH_NUCLEAR_FUSION', 'SPECIALIST_ENGINEER', 'YIELD_PRODUCTION', 3);

--==================================================================================================================================
-- Enlightenment Era Tech Tree Adjustment
--==================================================================================================================================
-- UPDATE Technologies SET GridX = GridX + 2 WHERE (Type='TECH_GLOBALIZATION' OR Cost> (SELECT Cost FROM Technologies WHERE Type='TECH_LASERS'))
-- AND EXISTS (SELECT * FROM Technologies WHERE Type='TECH_EE_WARSHIPS');

-- CREATE TRIGGER IF NOT EXISTS FutureAgeWithEnlightenmentEra
-- AFTER INSERT ON Technology_Flavors WHEN NEW.TechType = 'TECH_EE_WARSHIPS' AND NEW.FlavorType = 'FLAVOR_NAVAL_GROWTH'
-- BEGIN
-- 	UPDATE Technologies SET GridX = GridX + 2 WHERE Type='TECH_GLOBALIZATION' OR Cost> (SELECT Cost FROM Technologies WHERE Type='TECH_LASERS');
-- END;
--==================================================================================================================================
--==================================================================================================================================